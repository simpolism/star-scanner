...
async function main(): Promise<void> {
  ...
  scanner.on("start", (info: StartEventType) => {
    console.log(
      `Scan started: ${info.startDate.toDateString()} to ${info.endDate.toDateString()}`
    );
  });
  ...
  scanner.on("progress", (progress: ProgressEventType) => {
    console.log(
      ...
      }% complete (${progress.currentDate.toDateString()})`
    );
  });
  ...
  console.log(
    "Event data generated and saved. You can now view the data in the static HTML site."
  );
  return;
}
