...
export default function AstroChart({ event, width = 400, height = 400 }: AstroChartProps) {
  const chartRef = useRef<HTMLDivElement>(null);
  const chartInstanceRef = useRef<any>(null);

  useEffect(() => {
    // Check if astrology library is loaded
    if (typeof window !== 'undefined' && window.astrology && chartRef.current && event.planets) {
      // Generate chart ID
      const chartId = `chart-${new Date(event.date).getTime()}-${event.type.replace(/[^a-zA-Z0-9]/g, '')}`;
      ...
      try {
        ...
        if (chartRef.current) {
          chartRef.current.innerHTML = '<div class="chart-error">Unable to generate chart</div>';
        }
      }
    }
